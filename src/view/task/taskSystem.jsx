...
import TaskStatusCard from "../../components/task/TaskStatusCard/TaskStatusCard";
...
import {
  ...
  editTask,
  priorityChange,
} from "../../taskApiService/taskService"; // Functions to fetch data from backend
  ...
  useEffect(() => {
    if (tasks && tasks.length > 0) {
      ...
      const completedToday = tasks.filter(
        (task) =>
          task.completed && formatDate(task.completedDate) === formattedToday
      );
      // Filter tasks for pending tasks due today or before today
      const pendingTodayOrOverdue = tasks.filter(
        (task) => !task.completed && formatDate(task.dueDate) === formattedToday
      );
